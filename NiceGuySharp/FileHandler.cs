    ...
    internal class FileHandler
    {
        ...
        public static void DoChecks()
        {
            ...
            if (!File.Exists(OnGameStartTxt))
            {
                ...
                newfile.Close();
                const string content = "Chào các Sửu Nhi\nHi mấy c hó\nhi trẻ trâu\nQuẩy nào các muội\nFeed nào các e";
                var separator = new[] { "\n" };
            ...
            if (!File.Exists(OnGameEndTxt))
            {
                ...
                newfile.Close();
                const string content = "Mịa đánh ngu như ch\nTụi bay oc vừa thôi";
                var separator = new[] { "\n" };
                string[] lines = content.Split(separator, StringSplitOptions.None);
            ...
            if (!File.Exists(OnKillTxt))
            {
                ...
                newfile.Close();
                const string content = "/all Anh trùm thách đấu :D \n/all Anh chấp cả thế giới\n/all Anh Nam Xuyen Vũ trụ";
                var separator = new[] { "\n" };
                string[] lines = content.Split(separator, StringSplitOptions.None);
            ...
            if (!File.Exists(OnDoubleTxt))
            {
                ...
                newfile.Close();
                const string content = "/all Anh hay quá mà\n/all Các em quá gà\n/all Vô đối\nff đi mấy con gà";
                var separator = new[] { "\n" };
                string[] lines = content.Split(separator, StringSplitOptions.None);
            ...
            if (!File.Exists(OnTripleTxt))
            {
                ...
                newfile.Close();
                const string content = "oooh Anh đang xai tool!\nfk yeah";
                var separator = new[] { "\n" };
                string[] lines = content.Split(separator, StringSplitOptions.None);
